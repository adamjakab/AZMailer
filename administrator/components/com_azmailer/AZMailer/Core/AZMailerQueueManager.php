...
class AZMailerQueueManager {
	...
	public function getSentMailCount() {return($this->sentMailCount);}
	...
	*/
	...
	private function setup() {
		...
		if (empty($this->HELO)) {$this->HELO = 'localhost';}

		//SETUP - SMTP X-MAILER
		$this->SMTP_HEADER_XMAILER = $AZMAILER->getOption('smtp_header_x_mailer');
		if (empty($this->SMTP_HEADER_XMAILER)) {$this->SMTP_HEADER_XMAILER = $componentXmlData["name"] . ' v.'.$componentXmlData["version"];}

		//SETUP - SMTP RETURN-PATH (BOUNCE ADDRESS)
		$this->SMTP_HEADER_RETURN_PATH = $AZMAILER->getOption('smtp_header_return_path');
		if (empty($this->SMTP_HEADER_RETURN_PATH)) {$this->SMTP_HEADER_RETURN_PATH=null;}//sender address will be used
		...
		if (empty($this->SMTP_HEADER_MESSAGEID)) {$this->SMTP_HEADER_MESSAGEID='@'.$this->HELO;}
		if ($this->SMTP_HEADER_MESSAGEID[0] != '@') {$this->SMTP_HEADER_MESSAGEID = '@' . $this->SMTP_HEADER_MESSAGEID;}
