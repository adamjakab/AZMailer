...
class AZMailerNewsletterHelper {
	...
	public static function getNewsletterSubstitutedContentFromTemplate($tpl_id=0, $tpl_subst=null) {
		...
		if (!empty($tpl_id)) {
			...
			if (!is_object($SUBSTITUTIONS)) {$SUBSTITUTIONS = new \stdClass();}
			//
			$doc = new \DOMDocument();
	...
	public static function getNewsletterSendtoData($nl_sendto_selections, $nl_sendto_additional) {
		...
		if (!is_object($NL_ADDITIONAL)) {$NL_ADDITIONAL = new \stdClass();}
		if (!isset($NL_ADDITIONAL->subscribers) || !is_array($NL_ADDITIONAL->subscribers)){$NL_ADDITIONAL->subscribers = array();}
	...
	public static function getNLSubscribersForCategorySelections($NL_CATSEL) {
		...
		//WHERE - GEO LOCATIONS
		$whereLoc = array();
		if (!isset($NL_CATSEL->country)) {$NL_CATSEL->country = 0;}
		if (!isset($NL_CATSEL->region)) {$NL_CATSEL->region = 0;}
		if (!isset($NL_CATSEL->province)) {$NL_CATSEL->province = 0;}
		//
		if ($NL_CATSEL->country) {$whereLoc[] = 'res.nls_country_id = ' . $NL_CATSEL->country;}
		if ($NL_CATSEL->region) {$whereLoc[] = 'res.nls_region_id = ' . $NL_CATSEL->region;}
		if ($NL_CATSEL->province) {$whereLoc[] = 'res.nls_province_id = ' . $NL_CATSEL->province;}
		//
			...
			if (!is_null($file->uploadedfile) && $file->uploadedfile["error"]==0) {
				...
				if (in_array($file->EXTENSION, $allowedExtensions)) {
					...
					if (\JFile::upload($file->uploadedfile["tmp_name"], $dstfilepath)) {
						...
						if (!is_object($ATTACHMENTS)) {$ATTACHMENTS = new \stdClass();}
						if (!isset($ATTACHMENTS->attachments) || !is_array($ATTACHMENTS->attachments)) {$ATTACHMENTS->attachments = array();}
						//
